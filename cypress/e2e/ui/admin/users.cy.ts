import { Role, RoleValue, User, createUser, createUserList } from 'models/user';
import { checkPermissions } from '../shared/permission';
import {
  mockDeleteUser,
  mockGetAllUsers,
  mockGetUser,
...
describe('Admin User UI', () => {
  ...
  describe('Actions', () => {
    ...
    it('allows to edit the user correctly', () => {
      ...
      cy.getByTestID('admin-user-name').within(() => {
        cy.get('input').clear();
        cy.get('input').type(updatedUser.name);
      });

      cy.getByTestID('admin-user-email').within(() => {
        cy.get('input').clear();
        cy.get('input').type(updatedUser.email);
      });
      ...
      cy.wait('@updateUser').interceptFormData((formData) => {
        expect(formData.avatar).to.eq('image.jpg');
    ...
    it.only('deletes the user correctly', () => {
